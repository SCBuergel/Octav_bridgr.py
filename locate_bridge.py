...
# File path
xdai_bridge_csv = 'data/SAI_bridged.csv'
optimism_bridge_manual_csv = 'data/optimism_bridged_manual.csv'
octav_csv = 'data/all-time.csv'


def find_unmatched_outgoing_transfers(csv_file, address_asset_in):
    ...
    with open(csv_file, mode='r') as file:
        ...
        for row in reader:
            ...
            # Incoming arbitrum bridge transactions show up in reverse order (receiving account is shown as sender), so we have to reverse them:
            if network == 'Arbitrum' and row['To Address'] == '0x000000000000000000000000000000000000006e':
                asset_in_quantity = asset_out_quantity
                asset_out_quantity = ''
                asset_in_asset = asset_out_asset
                asset_out_asset = ''
                print(f'Line {row_number}: {asset_in_quantity} {asset_in_asset} incoming to {address} on Arbitrum')
            ...
            if asset_out_quantity and asset_out_asset:
                ...
                if key not in address_asset_in:
                    print(f"Line {row_number}: Asset OUT transfer of {asset_out_asset} before an IN transfer for {address} on {network}.")
    ...
    return address_asset_in

# Dictionary to keep track of asset transfers IN
all_address_asset_in = {}
all_address_asset_in = find_unmatched_outgoing_transfers(xdai_bridge_csv, all_address_asset_in)
# pprint.pprint(all_address_asset_in)
all_address_asset_in = find_unmatched_outgoing_transfers(optimism_bridge_manual_csv, all_address_asset_in)
# pprint.pprint(all_address_asset_in)
all_address_asset_in = find_unmatched_outgoing_transfers(octav_csv, all_address_asset_in)
# pprint.pprint(all_address_asset_in)
